...
# from atlas_utils.presenteragent import presenter_channel  
from concurrent.futures import ThreadPoolExecutor, as_completed #ADDED FROM CHATGPT ARYAN CK
...
if __name__ == "__main__":
    ...
    retry_attempt = 0
    MAX_ATTEMPT = 5
    connected = connected_drone.connect()
    while not connected and retry_attempt < MAX_ATTEMPT:
    ...
    #print(frame)
    drone = ParrotDrone.ParrotDrone(dist_to_target=80, run_duration=RUNTIME+5, connected_drone=connected_drone, nose_position=(540, 360))
    drone.start()
    ...
    while (time.time() - start_time) < RUNTIME:
      try:
          ...
          drone.dist_to_target = scaling_dist - person_to_track['dist']
          drone.nose_position = person_to_track['nose']
      ...
      #    connected_drone(Landing()).wait().success()
      #    connected_drone.disconnect()
      #    break
      except Exception:
          continue
